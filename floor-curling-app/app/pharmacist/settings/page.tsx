    ...
    return (
        <div className="min-h-screen bg-[#F2F2F7]">
            ...
            <main className="p-4 space-y-6 max-w-lg mx-auto">
                {/* 店舖基本資料卡片 */}
                {loading ? (
                    <div className="bg-white rounded-xl p-6 text-center text-gray-400">載入中...</div>
                ) : storeInfo ? (
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded font-mono">{storeInfo.id}</span>
                        </div>
                        <div className="p-5 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">店舖名稱</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">聯絡人</label>
                                <input
                                    type="text"
                                    value={storeInfo.contact_name}
                                    onChange={(e) => setStoreInfo({ ...storeInfo, contact_name: e.target.value })}
                                    placeholder="例如：王小明 店長"
                                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-colors"
                                />
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">連絡電話</label>
                                <input
                                    type="tel"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">地址</label>
                                <textarea
                                    value={storeInfo.address}
                                    onChange={(e) => setStoreInfo({ ...storeInfo, address: e.target.value })}
                                    placeholder="完整地址"
                                    rows={2}
                                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-colors resize-none"
                                />
                            </div>
                ...
                {/* 登出區塊 */}
                <div className="bg-red-50 rounded-xl overflow-hidden shadow-sm mt-8">
                    <button
